...
class MovieApi (retrofit: Retrofit) {
    ...
    fun getDetails(apiKey: String, id: Int) : Response<Movie?> {
        val call = movieService.getDetails(id, apiKey)
        Log.d(TAG, "${call.request().url()}")
        return call.execute()
    }

    fun getSimilarMovies(apiKey: String, id: Int): Response<SimilarMovies> {
    ...
    fun getMovieGenres(apiKey: String): Response<Genres> {
